 ...
 * context, seeds a starting question, and opens the global widget — without
 * coupling the page to the chat implementation itself.
 */
export function AskPowerAiButton({ context, question, label = "Ask Power AI" }: { context: PowerAIContext; question: string; label?: string }) {
  ...
  return (
    <button
      type="button"
      onClick={open}
      className="inline-flex h-7 shrink-0 items-center gap-1.5 rounded-md border border-violet-200 bg-violet-50 px-2.5 text-xs font-medium text-violet-800 hover:bg-violet-100"
    >
